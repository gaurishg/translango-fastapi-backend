...
translango_bucket = s3_resource.Bucket(BUCKET_NAME)
...
def upload_PIL_Image_to_s3(image: Image.Image) -> S3Object:
    name = hashlib.sha512(image.tobytes())
    ...
    in_mem_file.seek(0)
    return S3Object.from_orm(translango_bucket.put_object(Key=name, Body=in_mem_file))
